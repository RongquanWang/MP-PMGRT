...
class TripleModel_Catt(nn.Module):
    def __init__(self, dim=128, head=8, metric='cosine', p=0.2, text_neighbor=2, max_len=13):
        super().__init__()
        video_neighbor = 3
        text_neighbor = text_neighbor # UDIVA 设为10
        audio_neighbor = 3
        max_len = max_len # UDIVA 设为452
        act = nn.ReLU()
...
class TripleModel_Catt_ablation(nn.Module):
    def __init__(self, dim=128, head=8, metric='cosine', p=0.2, text_neighbor=2, max_len=13):
        super().__init__()
        video_neighbor = 3
        text_neighbor = text_neighbor # UDIVA 设为10
    ...
    def forward(self, video, text, wav2clip):
        ...
        # # x = self.mutliHead(xlstm_v, clip_clip, clip_wav)
        x = self.mutliHead(t, wav)
        x = self.lin_m(x)
        out = self.mlp_m(x)

        return out

class TripleModel_Catt_ablation2(nn.Module):
    def __init__(self, dim=128, head=8, metric='cosine', p=0.2, text_neighbor=2, max_len=13):
        super().__init__()
        video_neighbor = 3
        text_neighbor = text_neighbor # UDIVA 设为10
        ...
        # self.video_xlstm = xLSTM(768, 15)
        self.video_rnn = nn.GRU(input_size=768, hidden_size=384, num_layers=2, batch_first=True, bidirectional=True)
        self.video_lin_xlstm = nn.Sequential(nn.Dropout(p), nn.Linear(768, dim), act)
    ...
    def forward(self, video, text, wav2clip):
        ...
        # xlstm_v = self.video_xlstm(video)
        xlstm_v, _ = self.video_rnn(video)
        xlstm_v = self.video_lin_xlstm(xlstm_v).mean(dim=1)
